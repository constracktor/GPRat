        ...
        for (std::size_t n_train = START; n_train <= END; n_train += STEP)
        {
            ...
            for (std::size_t l = 0; l < LOOP; l++)
            {
                ...
                if (!use_gpu)
                {
                    ...
                    const auto cholesky_cpu = gp_cpu.cholesky();
                ...
                else
                {
                    ...
                    auto start_cholesky = std::chrono::high_resolution_clock::now();
                    const auto cholesky_gpu = gp_gpu.cholesky();
                    auto end_cholesky = std::chrono::high_resolution_clock::now();
                    cholesky_time = end_cholesky - start_cholesky;
                ...
                if (outfile.tellp() == 0)
                {
                    outfile << "Target,Cores,N_train,N_test,N_tiles,N_regressor,Opt_iter,Total_time,Init_time,"
