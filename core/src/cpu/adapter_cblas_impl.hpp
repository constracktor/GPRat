...
template <typename T>
mutable_tile_data<T> trsm_impl(
    const const_tile_data<T> &L,
    ...
    const BLAS_SIDE side_L)
{
    ...
    const T alpha = T(1);
    if constexpr (std::is_same_v<T, float>)
        cblas_strsm(CblasRowMajor, static_cast<CBLAS_SIDE>(side_L), CblasLower,
                    static_cast<CBLAS_TRANSPOSE>(transpose_L), CblasNonUnit, N, M, alpha, L.data(), N, A.data(), M);
    else
    ...
    const BLAS_TRANSPOSE transpose_B)
{
    ...
    const T beta = T(1);
    if constexpr (std::is_same_v<T, float>)
        cblas_sgemm(CblasRowMajor, static_cast<CBLAS_TRANSPOSE>(transpose_A),
                    static_cast<CBLAS_TRANSPOSE>(transpose_B), K, M, N, alpha, A.data(), K, B.data(), M, beta,
                    C.data(), M);
    else
...
template <typename T>
mutable_tile_data<T> trsv_impl(
    const const_tile_data<T> &L, const mutable_tile_data<T> &a, const int N, const BLAS_TRANSPOSE transpose_L)
{
    GPRAT_BENCHMARK_FORCE_EVICT(L.as_span());
    GPRAT_BENCHMARK_FORCE_EVICT(a.as_span());
    GPRAT_TIME_FUNCTION(&trsv);
    if constexpr (std::is_same_v<T, float>)
        cblas_strsv(CblasRowMajor, CblasLower, static_cast<CBLAS_TRANSPOSE>(transpose_L), CblasNonUnit, N, L.data(),
                    N, a.data(), 1);
    else
        cblas_dtrsv(CblasRowMajor, CblasLower, static_cast<CBLAS_TRANSPOSE>(transpose_L), CblasNonUnit, N, L.data(),
                    N, a.data(), 1);
    return a;
}

template <typename T>
mutable_tile_data<T> gemv_impl(
    const const_tile_data<T> &A,
    const const_tile_data<T> &a,
    ...
    const BLAS_TRANSPOSE transpose_A)
{
    ...
    const T beta = T(1);
    if constexpr (std::is_same_v<T, float>)
        cblas_sgemv(CblasRowMajor, static_cast<CBLAS_TRANSPOSE>(transpose_A), N, M, alpha, A.data(), M, a.data(), 1,
                    beta, b.data(), 1);
    else
        cblas_dgemv(CblasRowMajor, static_cast<CBLAS_TRANSPOSE>(transpose_A), N, M, alpha, A.data(), M, a.data(), 1,
                    beta, b.data(), 1);
    return b;
}

template <typename T>
mutable_tile_data<T> dot_diag_syrk_impl(
    const const_tile_data<T> &A, const mutable_tile_data<T> &r, const int N, const int M)
{
    GPRAT_BENCHMARK_FORCE_EVICT(A.as_span());
